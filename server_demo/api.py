...
def stats(length: int):
    ...
    return {
        ...
        'variable': [
            {'name': k, 'n': int(v), 'percent': float((current_df[f'label_{k}'].sum() - past_df[f'label_{k}'].sum()) / past_df[f'label_{k}'].sum() * 100 if past_df['label'].sum() else 0)} for k, v in result.items()
        ]
    }
